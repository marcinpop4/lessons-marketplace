...
export class Teacher extends Person {
    ...
    hourlyRates;
    constructor(id, firstName, lastName, email, phoneNumber, dateOfBirth, hourlyRates = []) {
     ...
     */
    addHourlyRate(hourlyRate) {
        // Remove any existing rate for this lesson type
        this.hourlyRates = this.hourlyRates.filter(rate => rate.type !== hourlyRate.type);
        // Add the new rate
     ...
     */
    deactivateHourlyRate(lessonType) {
     ...
     */
    reactivateHourlyRate(lessonType) {
        const rate = this.hourlyRates.find(rate => rate.type === lessonType);
        if (rate) {
