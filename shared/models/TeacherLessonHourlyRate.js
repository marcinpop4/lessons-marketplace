...
export class TeacherLessonHourlyRate {
    id;
    teacherId;
    type; // Maps to "type" column, using string to avoid import issues, will be one of LessonType enum values
    rateInCents; // Rate stored in cents (e.g., $45.50 = 4550 cents)
    createdAt;
    updatedAt;
    deactivatedAt; // When null, the rate is active; when set, the rate is deactivated
    constructor(id, teacherId, type, rateInCents, createdAt, updatedAt, deactivatedAt) {
        this.id = id;
     ...
     */
    getFormattedRate(locale = 'en-US', currency = 'USD') {
     ...
     */
    calculateCostForDuration(durationMinutes) {
