...
async function debugDockerContainers() {
    try {
        ...
        const { stdout: containerId } = await execAsync(`docker ps -aq --filter name=lessons-marketplace-server`);
        if (containerId.trim()) {
            ...
            try {
                ...
                try {
                    const { stdout: dbTest } = await execAsync(`docker exec ${containerId.trim()} curl -v database-${profileName}:5432`);
                    console.log(dbTest);
                }
                catch (dbError) {
                    ...
                    try {
                        ...
                        const { stdout: altDbTest } = await execAsync(`docker exec ${containerId.trim()} sh -c "nc -z -v database-${profileName} 5432 || echo 'Connection failed'"`);
                        ...
                        try {
                            const { stdout: pingTest } = await execAsync(`docker exec ${containerId.trim()} ping -c 3 database-${profileName}`);
...
`;
                ...
                try {
                    ...
                    try {
                        const { stdout: tsxVersion } = await execAsync(`docker exec ${containerId.trim()} which tsx`);
                        console.log(`tsx found at: ${tsxVersion}`);
                        const { stdout: prismaDebug } = await execAsync(`docker exec ${containerId.trim()} tsx server/prisma.debug.ts`);
                        ...
                        // Try with ts-node
                        try {
                            const { stdout: tsNodeOutput } = await execAsync(`docker exec ${containerId.trim()} npx ts-node server/prisma.debug.ts`);
                            ...
                            try {
                                ...
                                const { stdout: simpleDebug } = await execAsync(`docker exec ${containerId.trim()} node /app/simple-debug.js`);
                ...
                try {
                    ...
                    // Try with tsx first
                    try {
                        const { stdout: dbConnectionTest } = await execAsync(`docker exec ${containerId.trim()} tsx scripts/db-connection-test.ts`);
                        ...
                        // Try with ts-node
                        try {
                            const { stdout: tsNodeDbOutput } = await execAsync(`docker exec ${containerId.trim()} npx ts-node scripts/db-connection-test.ts`);
        ...
        // Check database logs
        const { stdout: dbContainerId } = await execAsync(`docker ps -aq --filter name=lessons-marketplace-db`);
